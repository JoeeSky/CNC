...
function checkName(value){
	if(value!=""){
		$.ajax({
			url:"clientManage/checkName.ajax?name="+value,//检查客户是否已经存在
			type:"get",
			dataType:"json",
		...
		$.ajax({
			url:"clientManage/checkName2.ajax?name="+value,//检查客户是否已申请只是还没审核
			type:"get",
			dataType:"json",
			success:function(data){ 
				if(!data.state){
					$("#nameCheck span").text("该客户已在审核队列中");
					$("#nameCheck").removeClass("hidden");
					nameValid = false;
				}else{
					$("#nameCheck").addClass("hidden");
...
var contactHTML='<div class="row" style="margin-bottom:20px;" id="c{row_index}">'
					+'<div class="col-sm-8">'
						+'<div class="input-group input-group-sm"><span class="input-group-addon">状态<span class="text-danger">*</span></span><select class="form-control input-sm" data-name="state"><option value="O">在职</option><option value="L">离职</option></select></div>'
						...
						+'<div class="input-group input-group-sm"><span class="input-group-addon">手机<span class="text-danger">*</span></span><input type="text" class="form-control" data-name="cellphone" maxlength="20"></div>'
						+'<div class="input-group input-group-sm"><span class="input-group-addon">职位<span class="text-danger">*</span></span><input type="text" class="form-control" data-name="position" maxlength="20"></div>'
