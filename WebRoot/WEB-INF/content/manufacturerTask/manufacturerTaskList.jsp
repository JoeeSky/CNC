	...
	<script>	
		$(document).ready(function() {
		    ...
		    $("#jqgrid").length > 0 && (t.jqGrid({
		    	...
		    	url:"manufacturerTask/getManufacturerTaskList.ajax",
		    	mtype:"GET",
		    	datatype:"json",
		    	colNames:['任务名','发布时间','发布方','接受方','状态','是否完成','操作'],
		    	height:410,
		    	rowNum:10,
        		...
        		gridComplete: function(){
        			...
        			for(var i=0;i < ids.length;i++){
        				...
        				var isEnd=$("#jqgrid").jqGrid('getCell',ids[i],'isEnd');
        				if(!cnc){
        					cnc='<button class="btn btn-success btn-xs">+选择机加工工厂</button>';
        					t.jqGrid('setRowData',ids[i],{cnc:cnc});
        				}
        				if(!state=="已接收"&&isEnd=="未完成"){
        					isEnd='<button class="btn btn-info btn-xs">确定完成</button>';
        					t.jqGrid('setRowData',ids[i],{isEnd:isEnd});
        				}
        				
        				alter = '<button class="btn btn-info btn-xs" onclick="location.href=\'userManage/userUpdateDo?tid='+ids[i]+'\'">修改</button>';
                        delet = '<button class="btn btn-danger btn-xs" onclick="$(\'#jqgrid\').delGridRow(\''+ids[i]+'\')">删除</button>';
                        t.jqGrid('setRowData',ids[i],{operation:alter+delet});
                    }
        		}
